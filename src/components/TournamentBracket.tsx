...
interface TournamentBracketProps {
  ...
  onRefresh: () => void;
  config: any;
  liveScores?: Record<number, { s1: string; s2: string }>;
}
...
export function TournamentBracket({ games, teams, config, liveScores }: TournamentBracketProps) {
  ...
  const generateAllColumns = () => {
    if (isProjection) {
      ...
      while (prevGames > 1) {
        ...
        const fakeMatches = Array(prevGames).fill(0).map(() => ({ ida: { _ghost_t1: null, _ghost_t2: null, match_id: Math.random(), round: currentRound }, volta: isHomeAway ? {} : null }));
    ...
    while (currentRound <= maxRound) {
        ...
        if (phaseGamesIda.length > 0) {
            ...
            // Definição explícita como any[] para permitir os ghost matches que nivelam a árvore
            const matches: any[] = phaseGamesIda.map(ida => {
                const volta = phaseGamesVolta.find(v => v.team_house_id === ida.team_out_id && v.team_out_id === ida.team_house_id);
                return { ida, volta };
            });
            
            // Lógica para forçar a simetria de caixas independentemente de quantos jogos existam no DB
            let step = isHomeAway ? Math.floor((currentRound - 90) / 2) : (currentRound - 90);
            let numK = config.knockoutTeams || 8;
            let expectedMatches = Math.max(1, numK / Math.pow(2, step + 1));
            
            while (expectedMatches < matches.length) expectedMatches *= 2;
            
            while (matches.length < expectedMatches) {
                matches.push({ ida: { _ghost_t1: null, _ghost_t2: null, match_id: Math.random(), round: currentRound }, volta: isHomeAway ? {} : null });
            }

            const title = expectedMatches === 1 ? "Final" : expectedMatches === 2 ? "Semis" : expectedMatches === 4 ? "Quartas" : expectedMatches === 8 ? "Oitavas" : "Fase";
  ...
  allColumns.forEach(col => {
    if (col.matches.length === 1) finalMatch = col; 
    else {
  ...
  return (
    <div className="relative w-full h-[100vh] min-h-[700px] flex flex-col items-center justify-center overflow-visible bg-background/50 rounded-xl pt-2 pb-6 border border-border/20">
      {isProjection && (
         <div className="bg-white/10 border border-white/20 text-white px-6 py-2 rounded-lg flex items-center gap-3 shadow-sm backdrop-blur-md shrink-0 mb-4">
      ...
      <div className="w-full flex-1 flex justify-center items-stretch px-1 md:px-2 gap-0 mt-4">
             ...
             return (
              <div key={`left-${colIndex}`} className="flex-1 flex flex-col h-full shrink-0">
                <div className="h-8 lg:h-10 flex items-center justify-center shrink-0 mb-2">
                   <span className="font-display text-[10px] md:text-xs lg:text-sm text-muted-foreground uppercase tracking-widest font-bold text-center drop-shadow-sm">{col.title}</span>
                </div>
                <div className="flex-1 flex flex-col w-full">
                   {pairs.map((pair, pairIndex) => (
                      <div key={pairIndex} className="flex-1 flex flex-col relative w-full justify-around px-2 md:px-3 lg:px-4 py-1 lg:py-2">
                          {pair.length === 2 && (
                              <>
                                  <div className="absolute right-0 top-[25%] bottom-[25%] w-2 md:w-3 lg:w-4 border-r-2 border-y-2 border-border/60 rounded-r-lg z-0" />
                                  <div className="absolute -right-2 md:-right-3 lg:-right-4 top-1/2 w-2 md:w-3 lg:w-4 border-t-2 border-border/60 z-0" />
                              </>
                          )}
                          {pair.length === 1 && <div className="absolute -right-2 md:-right-3 lg:-right-4 top-1/2 w-4 md:w-6 lg:w-8 border-t-2 border-border/60 z-0" />}
                          <div className="flex-1 flex flex-col justify-center w-full relative z-10 py-1"><BracketMatchCard matchObj={pair[0]} isFinal={false} getTeam={getTeam} liveScores={liveScores} /></div>
                          {pair.length > 1 && <div className="flex-1 flex flex-col justify-center w-full relative z-10 py-1"><BracketMatchCard matchObj={pair[1]} isFinal={false} getTeam={getTeam} liveScores={liveScores} /></div>}
                      </div>
                   ))}
                </div>
              </div>
          ...
          <div className="flex-[1.2] lg:flex-[1.5] flex flex-col h-full relative z-20 shrink-0">
            <div className="h-8 lg:h-10 flex items-center justify-center shrink-0 mb-2">
                <span className="font-display text-xs md:text-sm lg:text-base text-white uppercase tracking-widest font-black flex items-center gap-2 drop-shadow-sm">
            ...
            <div className="flex-1 flex flex-col items-center justify-center w-full px-2 md:px-3 lg:px-4">
              <div className="flex flex-col items-center justify-end flex-1 w-full pb-4 lg:pb-8">
                ...
                {championTeam ? (
                  <div className="bg-background/95 border border-white/50 px-4 md:px-6 py-2 md:py-3 rounded-xl shadow-[0_0_20px_rgba(255,255,255,0.2)] text-center flex flex-col items-center w-full max-w-[90%] backdrop-blur-md">
                    <span className="font-display font-black text-lg md:text-2xl lg:text-3xl text-white uppercase truncate w-full drop-shadow-md">{championTeam.name_player}</span>
              ...
              {finalMatch && (
                <div className="w-full shrink-0 relative z-10">
                   <BracketMatchCard matchObj={finalMatch.matches[0]} isFinal={true} getTeam={getTeam} liveScores={liveScores} />
                </div>
              )}
              <div className="flex flex-col items-center justify-start flex-1 w-full pt-6 lg:pt-10 relative">
                {!isProjection && thirdPlaceMatch && (
                  <div className="w-full max-w-[90%] border-t border-border/30 pt-6 relative animate-fade-in">
                    ...
                    <BracketMatchCard matchObj={{ ida: thirdPlaceMatch, volta: null }} isFinal={false} getTeam={getTeam} isBronze={true} liveScores={liveScores} />
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="flex-1 flex flex-col w-full">
                   {pairs.map((pair, pairIndex) => (
                      <div key={pairIndex} className="flex-1 flex flex-col relative w-full justify-around px-2 md:px-3 lg:px-4 py-1 lg:py-2">
                          {pair.length === 2 && (
                              <>
                                  <div className="absolute left-0 top-[25%] bottom-[25%] w-2 md:w-3 lg:w-4 border-l-2 border-y-2 border-border/60 rounded-l-lg z-0" />
                                  <div className="absolute -left-2 md:-left-3 lg:-left-4 top-1/2 w-2 md:w-3 lg:w-4 border-t-2 border-border/60 z-0" />
                              </>
                          )}
                          {pair.length === 1 && <div className="absolute -left-2 md:-left-3 lg:-left-4 top-1/2 w-4 md:w-6 lg:w-8 border-t-2 border-border/60 z-0" />}
                          <div className="flex-1 flex flex-col justify-center w-full relative z-10 py-1"><BracketMatchCard matchObj={pair[0]} isFinal={false} getTeam={getTeam} liveScores={liveScores} /></div>
                          {pair.length > 1 && <div className="flex-1 flex flex-col justify-center w-full relative z-10 py-1"><BracketMatchCard matchObj={pair[1]} isFinal={false} getTeam={getTeam} liveScores={liveScores} /></div>}
                      </div>
                   ))}
...
function BracketMatchCard({ matchObj, isFinal, getTeam, isBronze, liveScores }: any) {
  ...
  // Lógica de estado derivado (Lê o rascunho de edições na MatchInput)
  const liveIda = ida && liveScores?.[ida.match_id];
  const liveVolta = volta && liveScores?.[volta.match_id];
  const isLive = !!liveIda || !!liveVolta;
  
  let t1 = getTeam(ida?.team_house_id) || ida?._ghost_t1;
  let t2 = getTeam(ida?.team_out_id) || ida?._ghost_t2;
  
  let s1Main = null, s1Sub = null, s2Main = null, s2Sub = null, homeWins = false, outWins = false, isTie = false, pWinner = null;

  if (isHomeAway) {
      if (!isIdaFinished) {
          s1Main = liveIda?.s1 !== undefined && liveIda?.s1 !== "" ? liveIda.s1 : (ida?.status_game !== 'Pendente' ? ida?.goals_home : null);
          s2Main = liveIda?.s2 !== undefined && liveIda?.s2 !== "" ? liveIda.s2 : (ida?.status_game !== 'Pendente' ? ida?.goals_out : null);
      } else {
          s1Sub = ida.goals_home; 
          s2Sub = ida.goals_out;
          const isVoltaPending = !volta?.status_game || volta.status_game === 'Pendente';
          
          if(liveVolta && liveVolta.s1 !== "" && liveVolta.s2 !== "") {
              // no banco de dados de volta, house_id vira out_id
              if (volta.team_house_id === t1?.id) {
                 s1Main = liveVolta.s1; s2Main = liveVolta.s2;
              } else {
                 s1Main = liveVolta.s2; s2Main = liveVolta.s1;
              }
          } else {
          ...
          if (isVoltaFinished) {
              const agg1 = s1Sub + (s1Main || 0); const agg2 = s2Sub + (s2Main || 0);
              isTie = agg1 === agg2; pWinner = volta.penalty_winner_id;
              homeWins = agg1 > agg2 || (isTie && pWinner === t1?.id); outWins = agg2 > agg1 || (isTie && pWinner === t2?.id);
          }
      }
  } else if (!isGhost) {
      s1Main = liveIda?.s1 !== undefined && liveIda?.s1 !== "" ? liveIda.s1 : (ida?.status_game !== 'Pendente' ? ida?.goals_home : null);
      s2Main = liveIda?.s2 !== undefined && liveIda?.s2 !== "" ? liveIda.s2 : (ida?.status_game !== 'Pendente' ? ida?.goals_out : null);
      if (isIdaFinished) {
          isTie = s1Main === s2Main; pWinner = ida.penalty_winner_id;
          homeWins = s1Main > s2Main || (isTie && pWinner === t1?.id); outWins = s2Main > s1Main || (isTie && pWinner === t2?.id);
      }
  }

  const badgeText = isHomeAway ? (!isIdaFinished ? "IDA" : "VOLTA") : "";

  let cardStyles = "border-border/50 hover:border-white/50 transition-all shadow-md bg-card/95";
  if (isLive) {
      cardStyles = "border-white/80 shadow-[0_0_20px_rgba(255,255,255,0.2)] bg-background";
  } else if (isFinal && (isIdaFinished || isVoltaFinished)) {
      cardStyles = "border-white shadow-[0_0_20px_rgba(255,255,255,0.3)] ring-2 ring-white/50 bg-background";
  } else if (isFinal) {
  ...
  if (isGhost) {
    return (
      <div className="w-full h-full min-h-[60px] lg:min-h-[80px] max-h-[140px] relative rounded-xl border-2 border-dashed border-border/40 opacity-50 bg-transparent">
        {badgeText && <div className="absolute -top-3 lg:-top-3.5 left-1/2 -translate-x-1/2 bg-background border border-border/50 px-2 py-0.5 rounded-md text-[7px] lg:text-[9px] font-bold uppercase tracking-widest text-muted-foreground z-30 shadow-sm whitespace-nowrap">{badgeText}</div>}
  ...
  return (
    <div className={`w-full h-full min-h-[60px] lg:min-h-[80px] max-h-[140px] rounded-xl relative border-2 ${cardStyles}`}>
      {(badgeText || isLive) && (
         <div className={`absolute -top-3 lg:-top-3.5 left-1/2 -translate-x-1/2 ${isLive ? 'bg-white/10 border-white text-white shadow-[0_0_10px_rgba(255,255,255,0.2)]' : 'bg-muted border-border/50 text-muted-foreground'} border px-2 py-0.5 rounded-md flex items-center gap-1.5 text-[7px] lg:text-[9px] font-bold uppercase tracking-widest z-30 whitespace-nowrap`}>
           {isLive && <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />}
           {badgeText}
           {isLive && !badgeText && <span className="text-white">Ao Vivo</span>}
         </div>
      )}
...
function MatchRow({ player, score, subScore, isWinner = false, seed, isPenaltyWinner }: any) {
  return (
    <div className={`flex flex-1 items-center justify-between px-2 md:px-3 lg:px-4 py-1.5 lg:py-2 transition-colors ${isWinner ? "bg-white/5" : ""}`}>
      <div className="flex items-center gap-2 lg:gap-3 min-w-0">
        {player ? (
          <Shield className="h-5 w-5 md:h-6 md:w-6 lg:h-8 lg:w-8 shrink-0 drop-shadow-md" color={player.color || '#fff'} fill={player.color || 'transparent'} />
        ) : (
        ...
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-1.5 lg:gap-2">
            <span className={`text-xs md:text-sm lg:text-base font-display font-black leading-none truncate max-w-[60px] md:max-w-[100px] lg:max-w-[160px] ${isWinner ? "text-white drop-shadow-[0_0_5px_rgba(255,255,255,0.4)]" : player ? "text-foreground" : "text-muted-foreground"}`}>
              {player?.name_player || "A Definir"}
            </span>
            {isPenaltyWinner && (
               <span className="text-[7px] lg:text-[9px] text-white border border-white/40 bg-white/10 px-1 py-0.5 rounded tracking-widest uppercase shadow-sm font-bold">Pên</span>
            )}
          </div>
      ...
      <div className="flex items-center gap-1.5 lg:gap-2 shrink-0 pl-2">
        {subScore !== null && <span className="text-[9px] lg:text-[11px] text-muted-foreground/70 font-bold">({subScore})</span>}
        <span className={`font-display font-black text-sm md:text-base lg:text-xl ${isWinner ? "text-white drop-shadow-[0_0_5px_rgba(255,255,255,0.4)]" : "text-foreground/80"}`}>{score !== null ? score : "-"}</span>
      </div>
    </div>
  );
}